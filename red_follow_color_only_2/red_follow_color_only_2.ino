// ============================================================
//  RED LINE FOLLOWER — Color Sensor Only
...
void setup() {
  ...
  stopMotors();
  Serial.println("Ready — following red.");
}
...
void loop() {
  ...
  if (color == COLOR_RED) {
    if (irLeft == 1 && irRight == 1) {
      ...
      Serial.println("RED — both on tape, forward");
      moveForward();
    }
    return;
  }

  // --- BLUE: fully off red, use IR to steer back
  if (color == COLOR_BLUE) {
  ...
  if (irLeft == 1 && irRight == 1) {
    ...
    Serial.println("UNKNOWN — both IR lost, forward");
    moveForward();
  }
}
